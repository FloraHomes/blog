...
import { dataSideBar } from "./dummydatasidebar";
// ** Custom Components
// import Avatar from "@components/avatar";

// ** Reactstrap Imports
import { InputGroup, Input, InputGroupText } from "reactstrap";
import tempImg2 from "../../assets/card-3.jpg";

const BlogSidebar = () => {
  // ** States
  const [data, setData] = useState(dataSideBar);

  const renderRecentPosts = () => {
    return data.blogSidebar.recentPosts.map((post, index) => {
      return (
        <div
          key={index}
          className={classnames("d-flex", {
            "mb-2": index !== data.blogSidebar.recentPosts.length - 1,
          })}
        >
